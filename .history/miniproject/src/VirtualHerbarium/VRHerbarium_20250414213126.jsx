  ...
  useEffect(() => {
    ...
    const fetchPlants = async () => {
      try {
        ...
        // Add default model URLs if not present
        const plantsWithModels = data.plants.map(plant => ({
          ...plant,
          modelUrl: plant.modelUrl || `/models/${plant.id.toLowerCase().replace(/\s+/g, '-')}.glb`
        }));
        
        setPlants(plantsWithModels);
        setError(null);
      } catch (error) {
